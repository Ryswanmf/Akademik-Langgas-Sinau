@extends('layouts.admin', ['title' => 'Edit Titik GPS', 'header' => 'Edit Titik GPS & Waktu Presensi'])
...
<div class="space-y-6" x-data="locationForm({
    initialLat: {{ old('latitude', $attendanceLocation->latitude) }},
    initialLng: {{ old('longitude', $attendanceLocation->longitude) }},
    initialRadius: {{ old('radius_meters', $attendanceLocation->radius_meters) }},
    initialPlusCode: '{{ addslashes(old('plus_code', $attendanceLocation->plus_code ?? '')) }}'
})">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-slate-500 font-medium">Edit: {{ $attendanceLocation->name }}</span>
            </div>
            <h1 class="text-xl font-extrabold text-slate-900 tracking-tight mt-1">Edit Titik GPS & Pengaturan Waktu</h1>
            <p class="text-xs text-slate-500 mt-0.5">Perbarui koordinat peta, batas radius geofence, serta jam operasional presensi siswa.</p>
        </div>
    </div>

    <!-- Main Form Grid -->
    <form method="POST" action="{{ route('admin.attendance-locations.update', $attendanceLocation) }}" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-2xs space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $attendanceLocation->name) }}" required
            ...
            <div class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-2xs space-y-5">
                ...
                <div class="pt-3 border-t border-slate-100 flex items-start gap-3">
                    <div class="flex h-5 items-center">
                        <input type="checkbox" id="strict_radius" name="strict_radius" value="1" {{ old('strict_radius', $attendanceLocation->strict_radius) ? 'checked' : '' }}
            ...
            <div class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-2xs space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                        <div>
                            ...
                            <input type="time" id="in_start" name="in_start" value="{{ old('in_start', $attendanceLocation->in_start) }}" required
                        ...
                        <div>
                            ...
                            <input type="time" id="in_on_time_end" name="in_on_time_end" value="{{ old('in_on_time_end', $attendanceLocation->in_on_time_end) }}" required
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-bold text-slate-900 focus:bg-white focus:border-indigo-600 focus:outline-none">
                        ...
                        <div>
                            ...
                            <input type="time" id="in_late_end" name="in_late_end" value="{{ old('in_late_end', $attendanceLocation->in_late_end) }}" required
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-bold text-slate-900 focus:bg-white focus:border-indigo-600 focus:outline-none">
                ...
                <div class="pt-4 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                        <div>
                            ...
                            <input type="time" id="out_start" name="out_start" value="{{ old('out_start', $attendanceLocation->out_start) }}" required
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-bold text-slate-900 focus:bg-white focus:border-indigo-600 focus:outline-none">
                        ...
                        <div>
                            ...
                            <input type="time" id="out_end" name="out_end" value="{{ old('out_end', $attendanceLocation->out_end) }}" required
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-bold text-slate-900 focus:bg-white focus:border-indigo-600 focus:outline-none">
                        ...
                        <div>
                            ...
                            <input type="time" id="auto_alpa_time" name="auto_alpa_time" value="{{ old('auto_alpa_time', $attendanceLocation->auto_alpa_time) }}" required
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-bold text-slate-900 focus:bg-white focus:border-indigo-600 focus:outline-none">
                ...
                <div class="pt-4 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        @php
                            $selectedDays = old('working_days', $attendanceLocation->working_days ?? [1, 2, 3, 4, 5, 6]);
                            $daysMap = [
                                1 => 'Senin',
                                2 => 'Selasa',
                                3 => 'Rabu',
                ...
                <div class="pt-3 border-t border-slate-100 flex items-start gap-3">
                    <div class="flex h-5 items-center">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $attendanceLocation->is_active) ? 'checked' : '' }}
                               class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-600">
                    </div>
                    <div>
                        <label for="is_active" class="text-xs font-bold text-slate-900 cursor-pointer">
                            Titik Lokasi GPS Aktif
                        </label>
            ...
            <div class="sticky top-6 rounded-3xl border border-slate-200/80 bg-white p-5 shadow-2xs space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500">Klik peta atau seret pin merah untuk menggeser lokasi</p>
                    </div>
