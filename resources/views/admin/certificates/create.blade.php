@extends('layouts.admin', ['title' => 'Terbitkan Nilai & Sertifikat', 'header' => 'Penerbitan Nilai & Sertifikat'])

@section('content')
<div class="max-w-4xl mx-auto space-y-6" x-data="certificateForm()">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Formulir Penerbitan Nilai & Sertifikat</h2>
            <p class="text-xs text-slate-500">Input evaluasi 6 komponen nilai kompetensi siswa, unggah berkas sertifikat, dan tentukan status publikasi.</p>
        </div>
    ...
    <form action="{{ route('admin.certificates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-xs space-y-5">
            ...
            <div>
                ...
                <input 
                    type="text" 
                    name="name" 
                    value="{{ old('name', 'Sertifikat Kelulusan & Penilaian Kompetensi') }}" 
                    required 
                    placeholder="cth. Sertifikat Kompetensi Fullstack Web Development" 
                    class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
                >
                @error('name')
                    <p class="text-[11px] text-rose-500 mt-1">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input 
                        ...
                        value="{{ old('certificate_number', 'LS-CERT-' . date('Y') . '-' . rand(100, 999)) }}" 
                        required 
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="program" 
                        x-model="program"
                        value="{{ old('program', 'Teknologi Informasi') }}" 
                        required 
                        class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
                    >
                    @error('program')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Upload Berkas Sertifikat (PDF / Gambar)</label>
                    <input 
                        type="file" 
                        name="file" 
                        accept=".pdf,image/*" 
                    ...
                    >
                    <p class="text-[10px] text-slate-400 mt-1">Opsional saat draft. Format: PDF, JPG, PNG (Maks 5 MB).</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <label class="relative flex cursor-pointer rounded-2xl border p-4 transition-all" :class="isPublished ? 'border-emerald-500 bg-emerald-50/40 ring-2 ring-emerald-500/20' : 'border-slate-200 bg-white hover:border-slate-300'">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-xs font-bold text-slate-800">Publikasikan Sekarang</p>
                            <p class="text-[11px] text-slate-500 mt-0.5 leading-relaxed">
                                Nilai 6 kriteria dan berkas sertifikat <strong>langsung muncul</strong> di halaman portal siswa terkait.
                            </p>
                        </div>
                    </div>
                </label>
                ...
                <label class="relative flex cursor-pointer rounded-2xl border p-4 transition-all" :class="!isPublished ? 'border-amber-500 bg-amber-50/40 ring-2 ring-amber-500/20' : 'border-slate-200 bg-white hover:border-slate-300'">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500 mt-0.5 leading-relaxed">
                                Data <strong>disembunyikan dari siswa</strong> hingga admin siap mempublikasikannya nanti.
                            </p>
                        </div>
                    </div>
                </label>
        ...
        <div class="pt-2 flex items-center justify-end gap-3">
            ...
            >
                <span x-text="isPublished ? 'Simpan & Publikasikan ke Siswa' : 'Simpan sebagai Draft'">Simpan</span>
            </button>
        </div>
    </form>
</div>
...
function certificateForm() {
    return {
        ...
        program: '{{ old('program', 'Teknologi Informasi') }}',
        isPublished: {{ old('is_published', 1) ? 'true' : 'false' }},
        scores: {
            discipline: {{ old('score_discipline', 85) }},
            initiative: {{ old('score_initiative', 85) }},
            teamwork: {{ old('score_teamwork', 85) }},
            responsibility: {{ old('score_responsibility', 85) }},
            attitude: {{ old('score_attitude', 90) }},
            attendance: {{ old('score_attendance', 90) }},
        },

        onStudentChange() {
            const selectEl = document.querySelector('select[name="student_id"]');
            const selectedOpt = selectEl.options[selectEl.selectedIndex];
            if (selectedOpt && selectedOpt.dataset.program) {
                this.program = selectedOpt.dataset.program;
            }
            if (selectedOpt && selectedOpt.dataset.attendance) {
                const att = parseFloat(selectedOpt.dataset.attendance);
                if (!isNaN(att)) {
                    this.scores.attendance = att;
                }
            }
        },
