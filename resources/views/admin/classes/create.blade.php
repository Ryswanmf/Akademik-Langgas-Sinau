@extends('layouts.admin', ['title' => 'Tambah Kelas', 'header' => 'Tambah Kelas Pelatihan'])
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500">Tambahkan kelas dan program baru di LKP Langgas Sinau.</p>
        </div>
    ...
    <div class="rounded-3xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-xs">
        <form action="{{ route('admin.classes.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="cth. Web Development Fullstack Batch 1" class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">Program Keahlian *</label>
                <input type="text" name="program" value="{{ old('program') }}" required placeholder="cth. Teknologi Informasi / Desain Grafis" class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">Deskripsi Kelas</label>
                <textarea name="description" rows="4" placeholder="Deskripsi singkat kurikulum atau materi kelas..." class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">{{ old('description') }}</textarea>
            </div>
