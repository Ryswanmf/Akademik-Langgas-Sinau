@extends('layouts.admin', ['title' => 'Tambah Nilai', 'header' => 'Input Nilai Siswa'])

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Formulir Input Nilai</h2>
            <p class="text-xs text-slate-500">Catat evaluasi nilai tugas, praktik, atau ujian siswa.</p>
        </div>
        ...
        <form action="{{ route('admin.grades.store') }}" method="POST" class="space-y-4">
                ...
                <select name="student_id" required class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
                    ...
                    @foreach ($students as $s)
                        <option value="{{ $s->id }}" {{ old('student_id', $selectedStudentId) == $s->id ? 'selected' : '' }}>
                            {{ $s->user->name }} ({{ $s->student_number }}) - Kelas: {{ $s->class->name ?? 'Belum ada' }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="subject" value="{{ old('subject') }}" required placeholder="cth. Responsive CSS / Database MySQL" class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Nilai (0 - 100) *</label>
                    <input type="number" step="0.01" min="0" max="100" name="score" value="{{ old('score') }}" required placeholder="85.50" class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">Keterangan / Feedback Evaluasi</label>
                <textarea name="description" rows="3" placeholder="Umpan balik pencapaian kompetensi siswa..." class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">{{ old('description') }}</textarea>
            </div>
